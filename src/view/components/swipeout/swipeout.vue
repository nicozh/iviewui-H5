<template>
    <div :class="['i-swipeout-wrap', iClass]">
        <div class="i-swipeout-item" @touchstart="handlerTouchstart" @touchmove="handlerTouchmove" @touchend="handlerTouchend"
            :style="{transform:`translate(${this.data.position.pageX}px, 0)`}">
            <div class="i-swipeout-content">
                <slot name="content"></slot>
            </div>
            <div class="i-swipeout-button-right-group" v-if="actions.length > 0" @touchend.prevent="loop">
                <div class="i-swipeout-button-right-item" v-for="(item,index) in actions" :style="[setStyle(item),{width:item.width+'px'}]"
                    :key="index" :data-index="index" @click.capture="handlerButton(index)">
                    <i-icon v-if="item.icon" :type="item.icon" :size="item.fontsize" :color="item.color">
                    </i-icon>
                    {{item.name}}
                </div>
            </div>
            <div class="i-swipeout-button-right-group" @touchend.prevent="loop" @click.capture="handlerParentButton"
